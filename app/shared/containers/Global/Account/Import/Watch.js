...
import { Button, Checkbox, Divider, Form, Header, Icon, Label, List, Segment, Tab } from 'semantic-ui-react';
...
class GlobalModalAccountImportWatch extends Component<Props> {
  ...
  importAccounts = () => {
    ...
    selected.forEach((auth) => {
      ...
      actions.importWallet(account, authorization, false, false, 'watch', settings.blockchain.chainId, pubkey);
    });
    this.props.onClose();
  }
  isValid = (accounts) => {
