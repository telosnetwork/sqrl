...
import GovernanceTFVotingCandidatesTableRow from './Table/Row';

class GovernanceTFVotingCandidatesTable extends Component<Props> {
  constructor(props) {
    super(props);
    this.state = {
      query: false
  ...
  render() {
      ...
      baseTable = (
        <Table.Body key="FullResults">
          {fullResults.map((candidate) => {
            ...
            return (
              <GovernanceTFVotingCandidatesTableRow
                actions={actions}
    ...
    return (
      <Segment basic vertical>
        <Grid>
          <Grid.Column width={1} key="TFVotingCandidateSearch" textAlign="right">
            <Input
              icon="search"
...
export default translate('producers')(GovernanceTFVotingCandidatesTable);
